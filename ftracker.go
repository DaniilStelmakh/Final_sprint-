...
func main() {
	...
	lengthPool := 100
	countPool := 4
	// Демонтрация работы функции ShowTrainingInfo
	showTraninginfo := ShowTrainingInfo(action, trainingType, float64(duration), float64(weight), float64(height), lengthPool, countPool)
	// Получение информации о результатх тренировки
	fmt.Println(showTraninginfo)
